...
### Аудит действий (SQLite-логи)

- **Кто** — `telegram_id` (бот) или login/email (web)
- **Когда** — UTC дата/время события
- **Что** — действие в системе (команда, upload, запрос, ответ)
- **Текст** — введённый запрос пользователя и текстовый ответ OpenAI (для image-режима — метка/usage)
